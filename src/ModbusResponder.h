/*
  ModbusResponder - a minimal Modbus TCP responder for Arduino.

  Implements Read Holding Registers (function code 0x03) over TCP,
  built directly on the Modbus Application Protocol spec (modbus.org).
  It serves values from a register array that YOUR sketch owns and
  updates; the responder only handles framing and the FC03 reply.

  ---------------------------------------------------------------------
  SIZING THE BUFFER (optional but recommended)

  Define MODBUS_MAX_REGS *before* including this header to size the
  internal I/O buffer for your project, e.g. 2 for a two-channel sensor,
  up to 125 (the FC03 spec maximum). If left undefined it defaults to
  125, which reserves more RAM than most projects need:

      #define MODBUS_MAX_REGS 2
      #include <ModbusResponder.h>

  ---------------------------------------------------------------------
  WHY THIS LIBRARY IS HEADER-ONLY (no separate .cpp)

  A library .cpp is compiled as its own translation unit, separate from
  your sketch. A #define in your .ino can never reach code compiled in a
  .cpp, regardless of include order. Keeping serve() defined here, inline,
  means the buffer below is sized using whatever MODBUS_MAX_REGS is set to
  at *your* include site - the only way the customization above can work.
  Do not split this into a .cpp.

  ---------------------------------------------------------------------
  Copyright (c) 2026 Levi Monte
...
class ModbusResponder {
  public:
    // regs        - pointer to YOUR holding register array (you own/update it)
        ...
        _regBaseAddr(regBaseAddr)
    {
      ...
      // (e.g. a channel count is bumped without also bumping the buffer
      // sizing constant), clamp here rather than let serve() write past
      // the end of its own buffer.
    }

    // Call once per loop iteration when a connected client has data
    // available. Returns false if the connection should be dropped.
    bool serve(Client& client) {
      ...
      // --- Read the PDU (length includes the 1-byte unit ID already read) ---
